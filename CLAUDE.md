...
- Mouse reporting (`\x1b[?1000h` + SGR `?1006h`) is **toggled per-focus**: enabled in sidebar/modal, disabled in terminal focus. This keeps native drag-to-select / Cmd+Click on URLs working in the embedded pane without needing a modifier. In terminal focus the host terminal converts a plain wheel into arrow keys (alt-screen fallback), so the embedded app receives them as navigation — to scroll the litetree view in terminal focus use **Shift+wheel** (host sends Shift+Up/Down, intercepted in `onTerminalInput`) or Shift+↑/↓ / Shift+PageUp/PageDown.
